...
int main()
{
    // 0 means that the semaphore is to be shared between threads of a process
    // 1 is the initial value for the semaphore
    sem_init(&mutex, 0, 1);
...
{
    sem_wait(&mutex);   // decrements the semaphore value from 1 -> 0
    int x;
    ...
    sem_post(&mutex);   // increment the semaphore from 0 -> 1
}
